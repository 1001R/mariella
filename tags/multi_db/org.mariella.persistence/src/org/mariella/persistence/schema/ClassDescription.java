...
public class ClassDescription implements Serializable {
	...
	private final Map<String, PropertyDescription> hierarchyPropertyDescriptions = new HashMap<String, PropertyDescription>();
	private final Collection<ClassDescription> immediateChildren = new HashSet<ClassDescription>();
...
public void afterDeserialization(ClassLoader classLoader) {
	...
	try {
		...
		for(PropertyDescription pd : getPropertyDescriptions()) {
			pd.afterDeserialization(classLoader, bi);
		}
	} catch(ClassNotFoundException e) {
		throw new RuntimeException(e);
	} 
...
public void postInitialize(ClassDescriptionInitializationContext context) {
	...
	for(ClassDescription child : immediateChildren) {
		...
		Collection<String> ambigous = new HashSet<String>();
		for(PropertyDescription propertyDescription : child.getHierarchyPropertyDescriptions()) {
			if(!ambigous.contains(propertyDescription.getPropertyDescriptor().getName())) {
				PropertyDescription existing = hierarchyPropertyDescriptions.get(propertyDescription.getPropertyDescriptor().getName());
				if(existing == null) {
					hierarchyPropertyDescriptions.put(propertyDescription.getPropertyDescriptor().getName(), propertyDescription);
...
public Object getIdentity(Object object) {
	...
	if(identityPropertyDescriptions.length == 0) {
		...
		identityMap.put(identityPropertyDescriptions[0].getPropertyDescriptor().getName(), ModifiableAccessor.Singleton.getValue(object, identityPropertyDescriptions[0]));
