...
public boolean buildDescription() {
	if(attributeInfo.getColumnInfo() != null) {
		...
		if (propertyDescriptor == null) {
			throw new IllegalStateException("No PropertyDescriptor for class " + getClassDescription().getClassName() + " and attribute " + attributeInfo.getName());
		}
		...
		System.out.println("No column info for attribute " + getClassDescription().getClassName() + "." + attributeInfo.getName());
		return false;
	}
}
...
public void buildMapping() {
	...
	ColumnMapping columnMapping = new ColumnMapping(getClassMapping(), getPropertyDescription(), attributeInfo.getColumnInfo().isInsertable(), attributeInfo.getColumnInfo().isUpdatable(), readColumn, updateColumn);
	if(attributeInfo.getGeneratedValueInfo() != null) {
		if(attributeInfo.getGeneratedValueInfo().getStrategy() == GenerationType.AUTO) {
			if(attributeInfo.isId()) {
				...
				throw new UnsupportedOperationException("Auto generated columns are only supported for primary keys (" + columnMapping.toString() + ")");
			}
		} else if(attributeInfo.getGeneratedValueInfo().getStrategy() == GenerationType.SEQUENCE) {
			if(attributeInfo.getGeneratedValueInfo().getGenerator() == null || attributeInfo.getGeneratedValueInfo().getGenerator().isEmpty()) {
			...
			Sequence sequence = entityMappingBuilder.getPersistenceBuilder().getSequence(getClassMapping().getPrimaryUpdateTable().getName(), attributeInfo.getGeneratedValueInfo().getGenerator());
			if(sequence == null) {
				throw new IllegalStateException("The sequence '" + attributeInfo.getGeneratedValueInfo().getGenerator() + "' specified in " + getClassMapping().getClassDescription().getClassName() + "." + getPropertyDescription().getPropertyDescriptor().getName() + " does not exist!");
			}
...
private Column getColumn(TableInfo tableInfo) {
	...
	if(attributeInfo.getConverterName() != null) {
		converter = entityMappingBuilder.getPersistenceBuilder().getConverterRegistry().getNamedConverter(attributeInfo.getConverterName());
	} else {
		converter = entityMappingBuilder.getPersistenceBuilder().getConverterRegistry().getConverterForColumn(getPropertyDescription(), dti, dci);
	}
