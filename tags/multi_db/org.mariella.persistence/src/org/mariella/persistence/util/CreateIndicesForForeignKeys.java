...
public class CreateIndicesForForeignKeys {
	...
	private class Hint {
		...
		public boolean equals(Object obj) {
			if(!(obj instanceof Hint) || ((Hint)obj). table != table) {
				return false;
			}
			Hint other = (Hint)obj;
		...
		private void printCommaDelimitedColumnNames(PrintWriter printWriter) {
			...
			for(Column column : columns) {
				...
				printWriter.append(column.getName());
			}
			printWriter.flush();
		}
		...
		String buildBaseName() {
			String s = nameAbbreviator.abbreviateTableName(table.getName());
			for (Column col : columns) {
				s +="@" + nameAbbreviator.abbreviateColumnName(col.getName());
			}
			return s;
		}
	}
	...
	private boolean buildDropStatements = false;
...
private void beginProperty() {
	hintMap =  new HashMap<Table, Hint>();
}
...
private Table getTable(ClassMapping classMapping, Column column) {
	...
	if(classMapping instanceof JoinedClassMapping) {
		table = classMapping.getPrimaryUpdateTable() != null ? classMapping.getPrimaryUpdateTable() : ((JoinedClassMapping)classMapping).getJoinUpdateTable();
	} else {
	...
	if (table == null) {
		if(classMapping instanceof JoinedClassMapping) {
			table = classMapping.getPrimaryTable() != null ? classMapping.getPrimaryTable() : ((JoinedClassMapping)classMapping).getJoinTable();
		} else {
