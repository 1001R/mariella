...
class SavePointSupport {
	...
	private static class BeanAccessor<T> {
		...
		public PropertyDescriptor getPropertyDescriptor(String propertyName) {
			int index = Arrays.binarySearch(properties, propertyName, new Comparator<Object>() {
				@Override
		...
		public Object getValue(T object, String propertyName) {
			try {
				...
				throw new RuntimeException("Error reading property '"+propertyName+"' of class "+beanClass.getName()+".", ex);
		...
		public void setValue(T object, String propertyName, Object value) {
			try {
				...
				throw new RuntimeException("Error setting property '"+propertyName+"' of class "+beanClass.getName()+".", ex);
			} catch (IllegalAccessException ex) {
				throw new RuntimeException("Error setting property '"+propertyName+"' of class "+beanClass.getName()+".", ex);
			}
		}
	...
	private static class PropertyChangeModification implements Modification {
		...
		private PropertyChangeModification(Object object, String propertyName, Object oldValue, Object newValue) {
			this.object = object;
	...
	private static class IndexedPropertyChangeModification implements Modification {
		private Object object;
		private String propertyName;
		private Object oldValue;
		...
		private IndexedPropertyChangeModification(Object object, String propertyName, int index, Object oldValue, Object newValue) {
			this.object = object;
			this.propertyName = propertyName;
			this.oldValue = oldValue;
			this.newValue = newValue;
	...
	private static class ModificationTrackerMemento implements Modification {
		private IModificationTrackerMemento modificationTrackerMemento;
	...
	private ModificationTrackerListener propertyListener = new ModificationTrackerListener() {
		@Override
	...
	private AbstractModificationTrackerImpl modificationTracker;
	private boolean propertyListenerHooked = false;
	private List<SavePoint> savePoints = new ArrayList<SavePoint>();
	private ArrayList<Modification> modifications = new ArrayList<Modification>(30);
	...
	void deleteAllSavePoints() {
		modifications = new ArrayList<Modification>(30);
	...
	void rollbackToSavePoint(SavePoint savePoint) {
		int savePointIndex = savePoints.indexOf(savePoint);
		if (savePointIndex >= 0) {
			Map<Class<?>, BeanAccessor<Object>> accessorMap = new HashMap<Class<?>, BeanAccessor<Object>>();
			unkookPropertyTracker();
			List<ModificationTrackerListener> persistentListeners = modificationTracker.getPersistentListeners();
			List<ModificationTrackerListener> pl = new ArrayList<ModificationTrackerListener>(persistentListeners);
