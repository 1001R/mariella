...
public boolean buildDescription() {
	...
	if(attributeInfo.getReverseAttributeInfo() != null) {
		propertyDescription = new ReferencePropertyDescription(getClassDescription(), propertyDescriptor, attributeInfo.getReverseAttributeInfo().getName());
	} else {
	...
	if(attributeInfo.getJoinColumnInfos() != null && attributeInfo.getJoinColumnInfos().size() > 0) {
		...
		for(JoinColumnInfo jci : joinColumnInfos) {
			if(jci.getReferencedColumnName() == null || jci.getReferencedColumnName().length() == 0) {
				throw new IllegalStateException("No referenced column name specified for relationship " + getClassDescription().getClassName() + "." + attributeInfo.getName() + "!");
			}
			...
			TableInfo referencedUpdateTableInfo = referencedEntityMappingBuilder.getUpdateTableInfoForForeignKeyColumn(jci.getReferencedColumnName());
			...
			if(keyReadColumn == null) {
				throw new IllegalStateException("Join column " + referencedTable + "." + jci.getReferencedColumnName() + " does not exist!");
			}

			Column keyUpdateColumn = referencedUpdateTable.getColumn(jci.getReferencedColumnName());
			if(keyUpdateColumn == null) {
				throw new IllegalStateException("Join column " + referencedUpdateTable + "." + jci.getReferencedColumnName() + " does not exist!");
			}
			
			Table myTable = getEntityMappingBuilder().getPersistenceBuilder().getTable(getEntityMappingBuilder().getTableInfo(attributeInfo));
			Table myUpdateTable = getEntityMappingBuilder().getPersistenceBuilder().getTable(getEntityMappingBuilder().getUpdateTableInfo(attributeInfo));
			
			Column joinReadColumn = getEntityMappingBuilder().getPersistenceBuilder().getColumn(myTable, jci.getName(), keyReadColumn.getConverter());
			Column joinUpdateColumn = null;
			if (jci.isInsertable() || jci.isUpdatable()) {
				joinUpdateColumn = getEntityMappingBuilder().getPersistenceBuilder().getColumn(myUpdateTable, jci.getName(), keyUpdateColumn.getConverter());
			}
