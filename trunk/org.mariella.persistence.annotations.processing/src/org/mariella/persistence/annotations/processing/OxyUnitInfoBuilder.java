...
public class OxyUnitInfoBuilder {
	private PersistenceUnitParser persistenceUnitParser;
...
public void build() {
	try {
		...
		for (OxyUnitInfo info : getOxyUnitInfos()) {
...
private Map<Class<?>,List<Class<?>>> readAnnotatedClasses(OxyUnitInfo oxyUnitInfo, Class<?> ... annotationClasses) throws Exception {
	...
	List<Entry> entries = persistenceUnitParser.readEntries(oxyUnitInfo);
...
public List<OxyUnitInfo> getOxyUnitInfos() {
	return persistenceUnitParser.getOxyUnitInfos();
}
