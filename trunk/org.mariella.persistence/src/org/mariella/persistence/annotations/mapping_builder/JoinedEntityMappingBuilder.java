	...
	if(tableDefiningBuilder == this) {
		...
		if(joinTableInfo == null) {
			...
			for(PrimaryKeyJoinColumnInfo primaryKeyJoinColumnInfo : entityInfo.getPrimaryKeyJoinColumnInfos()) {
				...
				Column joinTableColumn = persistenceBuilder.getColumn(joinUpdateTable, primaryKeyJoinColumnInfo.getName(), primaryKeyColumn.getConverter()); 
				...
				((JoinedClassMapping)classMapping).getPrimaryKeyJoinColumns().getPrimaryKeyJoinColumns().add(primaryKeyJoinColumn);
			}
		} 
	}
}
...
private SelectableHierarchyEntityMappingBuilder getTableDefiningEntityMappingBuilder() {
	...
	while(root.getSuperEntityMappingBuilder() != null && root.getSuperEntityMappingBuilder() instanceof SelectableHierarchyEntityMappingBuilder) {
...
protected Column getUpdateColumn(DiscriminatorColumnInfo discriminatorColumnInfo) {
	if(updateTable != null) {
		if(persistenceBuilder.hasColumn(updateTable, discriminatorColumnInfo.getName())) {
			return getPersistenceBuilder().getColumn(updateTable, discriminatorColumnInfo.getName(), StringConverter.Singleton);
		}
	}
	return null;
}
