...
VResourceChangeListener resourceChangeListener = new VResourceChangeListener() {
	public void resourceRemoved(VResourceChangeEvent event) {
		if (event.getResource() instanceof PersonResource && event.isPersistentChange())
			refreshContent();
	}
...
void refreshContent() {
	...
	// build new view entries and add entries as VResourceRef referers (to keep resources in cache) 
