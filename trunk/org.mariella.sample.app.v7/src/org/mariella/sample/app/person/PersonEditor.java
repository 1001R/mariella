...
public void implementInit() {
	...
	// set mariella's selection provider implementation as the site's selection-provider 
	getSite().setSelectionProvider(bindingContext.getSelectionProvider());
	
	// add page selection extension
	Activator.getBindingFactory().addContextExtension(bindingContext, 
			new FormPageSelectionExtension(PersonEditor.this));
}
...
public void implementAddPages() {
	try {
		...
		addPage(new AddressesEditorPage(PersonEditor.this));
