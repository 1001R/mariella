...
public abstract class EntityMappingBuilder {
	...
	private Map<AttributeInfo, AttributeMappingBuilder<?>> attributeMappingBuilders = new HashMap<AttributeInfo, AttributeMappingBuilder<?>>();
...
public EntityMappingBuilder getSuperEntityMappingBuilder() {
	return entityInfo.getSuperEntityInfo() == null ? null : persistenceBuilder.getEntityMappingBuilder(entityInfo.getSuperEntityInfo());
}

public void buildDescription() {
	classDescription = persistenceBuilder.getPersistenceInfo().getSchemaDescription().getClassDescription(entityInfo.getClazz().getName());
	if(classDescription != null) {
		throw new IllegalStateException();
	}
	
	if(entityInfo.getSuperEntityInfo() != null) {
		superClassDescription = persistenceBuilder.getPersistenceInfo().getSchemaDescription().getClassDescription(entityInfo.getSuperclassInfo().getClazz().getName());
	}
	if(superClassDescription == null) {
		classDescription = new ClassDescription(persistenceBuilder.getPersistenceInfo().getSchemaDescription(), entityInfo.getClazz().getName());
	} else {
		classDescription = new ClassDescription(persistenceBuilder.getPersistenceInfo().getSchemaDescription(), superClassDescription, entityInfo.getClazz().getName());
	}
	persistenceBuilder.getPersistenceInfo().getSchemaDescription().addClassDescription(classDescription);

	List<PropertyDescription> identityPropertyDescriptions = new ArrayList<PropertyDescription>();
	for(AttributeInfo attributeInfo : entityInfo.getAttributeInfos()) {
		if(attributeInfo instanceof BasicAttributeInfo) {
			BasicAttributeInfo bai = (BasicAttributeInfo)attributeInfo;
	...
	if(!identityPropertyDescriptions.isEmpty()) {
		classDescription.setIdentityPropertyDescriptions(identityPropertyDescriptions.toArray(new PropertyDescription[identityPropertyDescriptions.size()]));
	}
}
...
public void buildMapping() {
	...
	persistenceBuilder.getPersistenceInfo().getSchemaMapping().setClassMapping(getClassDescription().getClassName(), classMapping);
...
public void buildRelationAttributeMappings() {
	for(PropertyDescription pd : getClassDescription().getPropertyDescriptions()) {
		if(pd instanceof RelationshipPropertyDescription && !getClassDescription().isInherited(pd)) {
			RelationAttributeInfo attributeInfo = (RelationAttributeInfo)getAttributeInfo(pd.getPropertyDescriptor().getName());
			getAttributeMappingBuilder(attributeInfo).buildMapping();
		}
	}
}
