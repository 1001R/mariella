...
public boolean buildDescription() {
	PropertyDescriptor propertyDescriptor = Introspector.Singleton.getBeanInfo(getEntityInfo().getClazz()).getPropertyDescriptor(attributeInfo.getName());
	...
	if(attributeInfo.getReverseAttributeInfo() != null) {
		pd = new CollectionPropertyDescription(getClassDescription(), propertyDescriptor, referencedClass.getName(), attributeInfo.getReverseAttributeInfo().getName());
	} else {
...
public void buildMapping() {
	...
	if(attributeInfo.getJoinTableInfo() != null) {
		...
		if(attributeInfo.getJoinTableInfo().getJoinColumnInfos().size() == 0) {
			throw new IllegalStateException(attributeInfoString + ": Must specify at least one join column for table " + attributeInfo.getJoinTableInfo() + ")!");
		}

		joinTable = getEntityMappingBuilder().getPersistenceBuilder().getTable(attributeInfo.getJoinTableInfo().getCatalog(), attributeInfo.getJoinTableInfo().getSchema(), attributeInfo.getJoinTableInfo().getName());
...
protected void createMappingWithJoinTable() {
	if(attributeInfo.getOrderByInfo() != null) {
		Column orderByColumn = getEntityMappingBuilder().getPersistenceBuilder().getColumn(joinTable, attributeInfo.getOrderByInfo().getOrderBy(), IntegerConverter.Singleton);
...
protected void createForeignKeyMapToOwner() {
	foreignKeyMapToOwner = new HashMap<Column, ColumnMapping>();
	for(JoinColumnInfo jci : attributeInfo.getJoinTableInfo().getJoinColumnInfos()) {
		...
		if(ownerColumn == null) {
			throw new IllegalStateException(attributeInfoString + ": The column " + ownerTable + "." + jci.getReferencedColumnName() + " does not exist!");
		}
		ColumnMapping ownerColumnMapping = getEntityMappingBuilder().getColumnMapping(ownerColumn);
		if(ownerColumnMapping == null) {
			throw new IllegalStateException(attributeInfoString + ": The column " + ownerTable + "." + jci.getReferencedColumnName() + " is not mapped by owner!");
		}
		
		Column joinColumn = getEntityMappingBuilder().getPersistenceBuilder().getColumn(joinTable, jci.getName(), ownerColumnMapping.getReadColumn().getConverter());
		foreignKeyMapToOwner.put(joinColumn, ownerColumnMapping);
	}
}

protected void createForeignKeyMapToContent() {
	EntityMappingBuilder referencedEntityMappingBuilder = getEntityMappingBuilder().getPersistenceBuilder().getEntityMappingBuilder(referencedClassMapping);
	foreignKeyMapToContent = new HashMap<Column, ColumnMapping>();
	for(JoinColumnInfo jci : attributeInfo.getJoinTableInfo().getInverseJoinColumnInfos()) {
		if(jci.getReferencedColumnName() == null || jci.getReferencedColumnName().isEmpty()) {
			throw new IllegalStateException(attributeInfoString + ": No referencedColumnName specified!");
		}
		TableInfo referencedTableInfo = referencedEntityMappingBuilder.getTableInfoForForeignKeyColumn(jci.getReferencedColumnName());
		...
		if(contentColumn == null) {
			throw new IllegalStateException(attributeInfoString + ": The column " + referencedTable + "." + jci.getReferencedColumnName() + " does not exist!");
		}
		...
		if(contentColumnMapping == null) {
			throw new IllegalStateException(attributeInfoString + ": The column " + referencedTable + "." + jci.getReferencedColumnName() + " is not mapped by content!");
		}
		
		Column joinColumn = getEntityMappingBuilder().getPersistenceBuilder().getColumn(joinTable, jci.getName(), contentColumnMapping.getReadColumn().getConverter());
