...
public JoinBuilder createJoinBuilder(SubSelectBuilder subSelectBuilder, TableReference myTableReference) {
	if(joinColumns != null) {
		...
		for(JoinColumn joinColumn : joinColumns) {
			...
			joinBuilder.setJoinType(joinColumn.getMyReadColumn().isNullable() ? JoinType.leftouter : JoinType.inner);
		}
		return joinBuilder;
	} else {
	...
	if(joinColumns == null) {
		...
		for(JoinColumn joinColumn : joinColumns) {
			joinBuilder.getConditionBuilder(joinColumn.getMyReadColumn()).and(
				BinaryCondition.eq(
						joinBuilder.getJoinedTableReference().createColumnReferenceForRelationship(joinColumn.getMyReadColumn()),
	...
	if(joinColumns != null) {
		for(JoinColumn joinColumn : joinColumns) {
			if(isInsert && joinColumn.isInsertable() || !isInsert && joinColumn.isUpdatable()) {
				...
				persistor.getClassMapping().getPrimaryPreparedStatementBuilder(persistor, this).getRow().setProperty(joinColumn.getMyUpdateColumn(), relatedValue);
			}
		}
	}
}
