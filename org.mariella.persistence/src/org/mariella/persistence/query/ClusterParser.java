	...
	private class FragmentParser {
		...
		public void parse() {
			if(pathIndex < currentPath.size()) {
				...
				} else if(!(pd instanceof RelationshipPropertyDescription)) {
					invalidProperty(pathIndex);
				} else {
					RelationshipPropertyDescription rpd = (RelationshipPropertyDescription)pd;
					...
					if(value != null) {
						...
						if(rpd instanceof ReferencePropertyDescription) {
							...
							for(Object entry : list) {
								if(!visitor.indexedProperty(currentEntity, currentClassDescription, (CollectionPropertyDescription)rpd, index++, entry)) {
									return;
								}
								FragmentParser parser = new FragmentParser(pathIndex + 1, rpd.getReferencedClassDescription(), entry);
								parser.parse();
							}
						} else {
